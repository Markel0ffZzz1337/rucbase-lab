...
void DiskManager::destroy_file(const std::string &path) {
    if (path2fd_.count(path)){
        throw std::runtime_error("File is still open, cannot be destroyed!");
    }
    if (!is_file(path)){
...
void DiskManager::close_file(int fd) {
    if (!is_file(fd2path_[fd])) {
        throw std::runtime_error("File does not exist:" + fd2path_[fd]);
    }
    if (!path2fd_.count(fd2path_[fd])) {
        throw std::runtime_error("File is not open:" + fd2path_[fd]);
    }
    close(fd);
